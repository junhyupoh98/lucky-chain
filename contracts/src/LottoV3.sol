...
contract LottoV3 is ERC721URIStorage, Ownable, VRFConsumerBaseV2 {
    ...
    uint256 public constant TICKET_PRICE_USD = 1e6; // $1 in 6 decimals (USDT/USDC)
    ...
    // Fee configuration: 15% total fee (10% team + 5% gas)
    uint16 public feeBps = 1500; // 15%
    uint16 public teamFeeBps = 1000; // 10%
    uint16 public gasFeeBps = 500; // 5%
    
    // Prize weights: 85% goes to prizes (70%, 10%, 20%)
    uint16 public w1 = 7000; // 70% of prize pool
    uint16 public w2 = 1000; // 10% of prize pool
    uint16 public w3 = 2000; // 20% of prize pool

    address payable public teamTreasury;
    address payable public gasTreasury;
    ...
    event FeeConfigUpdated(uint16 feeBps, uint16 teamFeeBps, uint16 gasFeeBps, uint16 w1, uint16 w2, uint16 w3);
    ...
    error DuplicateNumbers();

    constructor(
        address vrfCoordinator,
        uint64 subscriptionId,
        bytes32 keyHash,
        address payable teamTreasuryAddress,
        address payable gasTreasuryAddress,
        address usdtAddress,
        address usdcAddress
    )
        ERC721("Lucky Chain Lottery V3", "LCLV3")
        Ownable(msg.sender)
        VRFConsumerBaseV2(vrfCoordinator) 
    {
        require(teamTreasuryAddress != address(0), "Team treasury required");
        require(gasTreasuryAddress != address(0), "Gas treasury required");
        ...
        teamTreasury = teamTreasuryAddress;
        gasTreasury = gasTreasuryAddress;
    ...
    function setTeamTreasury(address payable newTreasury) external onlyOwner {
        require(newTreasury != address(0), "invalid treasury");
        teamTreasury = newTreasury;
    }

    function setGasTreasury(address payable newTreasury) external onlyOwner {
        require(newTreasury != address(0), "invalid treasury");
        gasTreasury = newTreasury;
    }

    function updateFeeConfig(
        uint16 newFeeBps,
        uint16 newTeamFeeBps,
        uint16 newGasFeeBps,
        uint16 newW1,
        uint16 newW2,
        uint16 newW3
    ) external onlyOwner {
        require(newTeamFeeBps + newGasFeeBps == newFeeBps, "Fee split mismatch");
        require(uint256(newW1) + uint256(newW2) + uint256(newW3) == 10_000 - newFeeBps, "Weights must equal prize pool");
        
        feeBps = newFeeBps;
        teamFeeBps = newTeamFeeBps;
        gasFeeBps = newGasFeeBps;
        w1 = newW1;
        w2 = newW2;
        w3 = newW3;
        
        emit FeeConfigUpdated(newFeeBps, newTeamFeeBps, newGasFeeBps, newW1, newW2, newW3);
    }
    ...
    // Manual number selection - user chooses 6 numbers, lucky number is auto-generated
    function buyTicketManual(
        bool useUSDT,
        ...
        // Generate lucky number (must not be in main numbers)
        uint8 luckyNumber = _generateLuckyNumber(numbers, nextTicketId);
    ...
    // Auto number selection - all numbers generated automatically
    ...
    // Batch purchase with auto-generated numbers
    function buyTicketsAuto(
        bool useUSDT,
        uint256 count,
    ...
    function _validateNumbers(uint8[MAIN_NUMBERS] calldata numbers) internal pure {
        bool[MAX_NUMBER + 1] memory used;
        
        for (uint256 i = 0; i < MAIN_NUMBERS; i++) {
            uint8 num = numbers[i];
            if (num < 1 || num > MAX_NUMBER) revert InvalidNumbers();
            if (used[num]) revert DuplicateNumbers();
            used[num] = true;
            
            // Check if sorted
            if (i > 0 && numbers[i] <= numbers[i-1]) revert InvalidNumbers();
        }
    }

    function _generateLuckyNumber(uint8[MAIN_NUMBERS] calldata mainNumbers, uint256 seed) internal view returns (uint8) {
        bool[MAX_NUMBER + 1] memory used;
        for (uint256 i = 0; i < MAIN_NUMBERS; i++) {
            used[mainNumbers[i]] = true;
        }
    ...
    function _evaluateTickets(
        ...
        for (uint256 i = 0; i < tickets.length; i++) {
            ...
            (uint8 matched, bool luckyMatch) = _countMatches(picked, winning, ticketLuckyNumber[ticketId], lucky);
    ...
    function _countMatches(
        ...
        luckyMatch = (playerLucky == winningLucky);
    }
    ...
    function _finalizePayoutsForToken(Round storage r, bool isUSDT) internal {
        ...
        // Split fees: team (10%) and gas (5%)
        uint256 teamFee = (gross * teamFeeBps) / 10_000;
        uint256 gasFee = (gross * gasFeeBps) / 10_000;
        uint256 baseAmt = gross - teamFee - gasFee;

        // Prize distribution: 70%, 10%, 20% of prize pool (85% of gross)
        ...
        if (teamFee > 0 && teamTreasury != address(0)) {
            token.safeTransfer(teamTreasury, teamFee);
        }
        if (gasFee > 0 && gasTreasury != address(0)) {
            token.safeTransfer(gasTreasury, gasFee);
        }
    }
